...
-- WHERE M_SUB.MENU_ID = 1103 // 하위 메뉴의 메뉴아이디가 1103인 것의 부모를 찾아라 
...
;
-- Europe에서 근무중인 모든 사원의 이름과 근무중인 도시를 조회한다.
SELECT E.FIRST_NAME 
	 , L.CITY 
  FROM EMPLOYEES E
 INNER JOIN DEPARTMENTS D
...
SELECT COUNTRY_ID
  FROM COUNTRIES
 WHERE REGION_ID = Europe의 RegionID 
 ;
--
SELECT REGION_ID
  FROM REGIONS 
...
-- 직무가 변경된 사원들의 사원번호, 이름, 급여, 현재 수행중인 직무의 이름, 과거에 근무했던 부서의 이름,
-- 현재근무중인 부서의 이름을 조회한다.
...
-- 사원의 이름과 성, 직무 아이디, 직무의 이름, 급여, 최대 급여, 최소 급여를 조회한다.
SELECT E.FIRST_NAME
	 , E.LAST_NAME
     , J.JOB_ID
     , J.JOB_TITLE
...
--1. 118번 사원이 근무중인 부서의 지역번호를 조회한다.
SELECT LOCATION_ID
  FROM DEPARTMENTS
 WHERE DEPARTMENT_ID = 118번 사원이 근무중인 부서의 번호
;
SELECT DEPARTMENT_ID
  FROM EMPLOYEES
 WHERE EMPLOYEE_ID = 118  --30
;
SELECT LOCATION_ID
  FROM DEPARTMENTS
 WHERE DEPARTMENT_ID = (SELECT DEPARTMENT_ID
  						  FROM EMPLOYEES
 						 WHERE EMPLOYEE_ID = 118)  --1700
; 						 
--2. 118번 사원이 근무중인 지역 번호로 도시명을 조회한다.
SELECT CITY
  FROM LOCATIONS
 WHERE LOCATION_ID = 118번 사원이 근무중인 지역의 번호(1700)
;
...
SELECT DEPARTMENT_ID
  FROM DEPARTMENTS
 WHERE LOCATION_ID = 'Seattle'의 지역번호 
;
...
SELECT DISTINCT JOB_ID
 ...
 WHERE DEPARTMENT_ID IN (SELECT DEPARTMENT_ID
 						 ...
 						 WHERE LOCATION_ID = (SELECT LOCATION_ID
 											   ...
 											   WHERE CITY = 'Seattle'))
;
--3. 'Seattle'에존재하는 부서에서 근무하는 사원들의 직무명
...
-- 이 때, 사원의 수가 2이상인것과 급여의 총합이 20000 이상인 것만 조회한다.
SELECT DEPARTMENT_ID
	 , COUNT(EMPLOYEE_ID)
	 , SUM(SALARY)
  FROM EMPLOYEES
...
-- 급여가 10000이상이거나 커미션을 받는 사원의 급여, 커미션 비율을 조회한다.
SELECT SALARY
	  ,COMMISSION_PCT
  FROM EMPLOYEES
 WHERE SALARY >= 10000
...
-- 급여가 5000 이상 10000 미만이거나 직무아이디가 IT_PROG 인 사원의 급여와 직무아이디를 조회한다.
SELECT SALARY
	  ,JOB_ID
  FROM EMPLOYEES
...
-- 상사사원번호가 101번 이거나 103번인 사원 중 급여가 5000이상인 사원의 상사사원 번호와 급여를 조회한다.
SELECT MANAGER_ID
	  ,SALARY
  FROM EMPLOYEES
...
-- 이름이 STEVEN인 사원의 이름과 성,급여, 근무부서번호를 조회한다.
SELECT FIRST_NAME
	  ,LAST_NAME
	  ,SALARY
  FROM EMPLOYEES
...
SELECT LOCATION_ID
 ...
 WHERE CITY = 'Seattle'
;
-- 101번 사원의 직무변경 이력 중 직무 시작일, 직무 종료일, 직무아이디, 근무 부서 아이디를 조회한다.
...
-- 커미션 비율이 0.2 보다 작은 사원의 급여, 커미션 비율을 조회한다.
-- 급여 오름차순 정렬한다.
SELECT SALARY
	  ,COMMISSION_PCT
  FROM EMPLOYEES
